    ...
    public function render_form()
    {
        ...
        <form class="contact-form" id="contact-form" method="post" action="#contact-form">
            ...
            <p>
                ...
                <input required type="text" name="contact-name" id="contact-name" value="<?php if (isset( $_POST['contact-name'] )) {
                    echo esc_attr( $_POST['contact-name'] );
} ?>">
                <?php if ($this->has_error( 'contact-name' )) :
?><span class="input-error"><?php _e( 'Dies ist ein Pflichtfeld', 'TEXTDOMAIN' ) ?></span><?php
endif; ?>
            </p>

            <p>
                <label class="label--required" for="contact-email"><?php _e( 'E-Mail', 'TEXTDOMAIN' ) ?></label>
                <input required type="email" name="contact-email" id="contact-email" value="<?php if (isset( $_POST['contact-email'] )) {
                    echo esc_attr( $_POST['contact-email'] );
} ?>">
                <?php if ($this->has_error( 'contact-email' )) :
?><span class="input-error"><?php _e( 'Dies ist ein Pflichtfeld', 'TEXTDOMAIN' ) ?></span><?php
endif; ?>
            </p>
            ...
            <p>
                ...
                <input type="text" name="contact-phone" id="contact-phone" value="<?php if (isset( $_POST['contact-phone'] )) {
                    echo esc_attr( $_POST['contact-phone'] );
} ?>">
                <?php if ($this->has_error( 'contact-phone' )) :
?><span class="input-error"><?php _e( 'Dies ist ein Pflichtfeld', 'TEXTDOMAIN' ) ?></span><?php
endif; ?>
            </p>

            <p>
                <label class="label--required" for="contact-message"><?php _e( 'Nachticht', 'TEXTDOMAIN' ) ?></label>
                <textarea name="contact-message" id="contact-message"></textarea>
                <?php if ($this->has_error( 'contact-message' )) :
?><span class="input-error"><?php _e( 'Dies ist ein Pflichtfeld', 'TEXTDOMAIN' ) ?></span><?php
endif; ?>
            </p>
